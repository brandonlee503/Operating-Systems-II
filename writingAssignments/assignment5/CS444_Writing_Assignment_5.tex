...
One of the biggest core objectives of an operating system is to store and manage files in some meaningful, organized matter.  A system of logical hierarchy is necessary in order to establish such a regulation.  Without a file system, data would be universally held within a central area and identification of different data types would be a major disaster.  The congregation of these common objectives to prevent such a disaster results in a system to separate data into multiple pieces and assigning names to these pieces for accessibility and identification.\\

\subsection{Similarities}

Commonalities that are universally shared among the three file systems can be fundamentally brought down to disk based file organization.  Conceptually, many of the paradigms of file organization remain the same at this level - data is labeled with a particular type and subsequently stored somewhere on the hard disk.  With this in mind, there are many flaws that arise because of such a paradigm.\\

Firstly, both Unix and Windows file systems can become corrupted after misuse or other factors such as damaged files.  However it is notable that while this flaw is present in both operating systems, each operating system's file system has good security.  More specifically, Windows with its NTFS file system has Access Control Lists, which hold access control entries that identify trustees and enables access rights.\cite{mwi12}  On the other hand, Linux utilizes read, write, and default bits instead to grant such access rights to users.\\

Secondly, a major fault and commonality which both file systems on Windows and Unix face are hard disk fragmentation.\cite{mwi12}  Whenever files are deleted among a hard disk, there are empty spaces of memory left within the file system.  While these gaps can be filled with future files, the whole process of sifting through bits of memory and bits of spacious gaps can be problematic for efficiency.\\

Finally, another commonality shared between Unix systems and Windows is the ability to utilize multiple file systems.  In the Unix realm, the most common file system types are ext3 and ext4, with the legacy ext2.  While in Windows we see NTFS, along with the legacy Fat32 and Fat16.\cite{lkd13}\\

\subsection{Differences}

There are a multitude of differences among the file system implementations between Unix systems and Windows.  Firstly, the arguably most user observable difference lays within case sensitivity.  Unix has a case sensitive file system while Windows is not as meticulous on case sensitivity.  In reality however, Windows supports both through its NtCreateFile API.\cite{mwi12}  Another character related difference between the two operating systems is internationalization.  The Linux file system APIs support UTF-8 while Windows supports UTF-16.\cite{mwi12}  This can cause problems in the domain of moving data between the two operating systems as file names can be encoded in multiple ways.\\

There are many minor differences between the visual representation of file systems between these operating systems. In continuation of this theme, Unix systems use mount points while Windows references drive letters.  Unix uses forward slashes while Windows utilizes back slashes.\\

Directories in Unix are facilitated through directory entry objects, or dentrys.  A dentry is essentially the specific component in a path.  Dentrys can encompass all files in a path, including files as well.  The following example of a dentry object with its various members.\cite{mwi12} Take note of the data it holds and the similarities and differences it shares to the discussion below.\\

\lstinputlisting[caption=Linux Kernel Dentry Object, style=customc]{dentryExample.c}

Among these minor differences lie much more diverse differences, specifically with file sharing and permissions.  The concept of inodes arises in this situation.  In Unix, a file name is essentially a pointer to an inode, which is the location of which the data for the file is actually stored.\cite{lkd13}  This paradigm allows for the existence of multiple filenames that reference the same data.  These are known as hard links.  Another benefit to this paradigm is that a user can delete a currently opened file, this action is known as unlinking.  When unlinking occurs, the directory entry is deleted, but other processes are not affected by this as the file hangs on the inode until no more processes are referencing it.  In Windows, filenames are directly attached to the data, which results in the user being unable to deleting and renaming files while they are open.  The following example is the representation of the operations an inode makes.\cite{lkd13} Take note of the various features provided.\\

\lstinputlisting[caption=Linux Kernel Inode Operations Object, style=customc]{inodeOperation.c}

\subsection{Similarities/Differences - Why?}

The arching similarities between the various file systems can be seen in the core functionality of a file system.  Establishing a directory hierarchy for file organization is one commonality Unix and Windows share.  Because of this, both Unix and Windows design their file systems around this one paradigm and both benefit and suffer consequences because of it.\\

The spanning differences between Unix systems and Windows file systems can be traced down to their design paradigms.  In the examples discussed above, files in Unix are referenced by their inode rather than their file name in Windows.  This can be traced down to how Unix's design paradigm focuses on modularity, whereas Windows revolves around easy user interactivity, thusly having files solely based on their filenames.\\

\newpage
\bibliographystyle{plain}
\bibliography{CS444_Writing_Assignment_5}
\end{document}
